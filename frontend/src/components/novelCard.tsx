  ...
  return (
    <div className={"rounded-md bg-gradient-to-r from-blue-200 to-blue-500 flex flex-col gap-2 p-3 items-center w-70"}>
      ...
      <div className="flex">
        ...
        <span className="font-medium dark:text-black">{Math.floor(likes * 100)}%</span>
      </div>
    ...
    <div className="flex justify-start gap-3">
      ...
      <div className="flex flex-col justify-between">
        ...
        <div className="flex">
          ...
          <span className="font-medium">{Math.floor(likes * 100)}%</span>
        </div>
        ...
        <div className="flex justify-start gap-2 mt-8 flex-wrap">
          {genres.map(item => (
            <Link href={`/series?sortBy=name&filter=${item}`} key={item} className="px-4 py-1 ease-in-out duration-300 capitalize text-sm font-medium rounded-md border hover:text-sky-500 hover:border-sky-500 dark:text-gray-400 dark:hover:text-sky-500">{item}</Link>
          ))}
        </div>
      </div>
    </div>
